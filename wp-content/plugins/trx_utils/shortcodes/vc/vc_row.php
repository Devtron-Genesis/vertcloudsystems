...
if (function_exists('vc_map_get_attributes')) {						// New VC
	 ...
	 * @var $scheme		- added by Invetex
	 * @var $inverse	- added by Invetex
	 * @var $atts
	 * @var $el_class
	 * @var $full_width
	 * @var $full_height
    ...
    if ( isset($disable_element) and ( $disable_element === 'yes' ) ) {
        return '';
    }
	...
	// Added by Invetex:
	// Color scheme
	if ($scheme && !invetex_param_is_off($scheme) && !invetex_param_is_inherit($scheme)) $css_classes[] = 'scheme_'.esc_attr($scheme);
	// Inverse colors
	if ($inverse && !invetex_param_is_off($inverse)) $css_classes[] = 'inverse_colors';
	...
	$output .= '</div>';
	$output .= $after_output;
	$output .= $this->debugComment( $this->getShortcode() );
	...
	// Add Invetex color scheme in the row
	if ($scheme && !invetex_param_is_off($scheme) && !invetex_param_is_inherit($scheme)) $css_class .= ' scheme_'.esc_attr($scheme);
	...
	} ?> <?php invetex_show_layout($style); ?>><?php
	echo wpb_js_remove_wpautop( $content );
	?></div><?php invetex_show_layout($this->endBlockComment('row'));
	if ( ! empty( $full_width ) ) {
