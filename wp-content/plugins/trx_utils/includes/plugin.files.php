...
if (!function_exists('trx_utils_get_theme_option')) {
    function trx_utils_get_theme_option($prm) {
        ...
        if (function_exists('invetex_get_theme_option')) {
            return invetex_get_theme_option($prm);
        }
        return null;
    }
}
...
if (!function_exists('trx_utils_get_option')) {
    function trx_utils_get_option($name, $defa='', $strict_mode=true) {
        global $TRX_UTILS_STORAGE;
        $rez = $defa;
        $part = '';
        ...
        if ( isset($TRX_UTILS_STORAGE['options']) && !isset($TRX_UTILS_STORAGE['options'][$name]) && $strict_mode ) {
            $s = debug_backtrace();
            ...
            $s = array_shift($s);
            echo '<pre>';
            echo esc_html(sprintf(__('Undefined option "%s" called from:', 'trx_utils'), $name));
            if (function_exists('trx_utils_debug_dump_screen'))
                trx_utils_debug_dump_screen($s);
            else
                print_r($s);
            echo '</pre>';
            die();
        ...
        } else if (isset($TRX_UTILS_STORAGE['options'][$name]['val'])) {
            if (empty($part))
                $rez = $TRX_UTILS_STORAGE['options'][$name]['val'];
            else if (isset($TRX_UTILS_STORAGE['options'][$name]['val'][$part]))
                $rez = $TRX_UTILS_STORAGE['options'][$name]['val'][$part];
        }
        return $rez;
    }
}
...
if ( !function_exists( 'trx_utils_enqueue_googlemap' ) ) {
    function trx_utils_enqueue_googlemap() {
        if (trx_utils_is_on(trx_utils_get_option('api_google_load'))) {
            $api_key = trx_utils_get_option('api_google');
            wp_enqueue_script( 'google-maps', trx_utils_get_protocol().'://maps.googleapis.com/maps/api/js'.($api_key ? '?key='.$api_key : ''), array(), null, true );
        }
    }
}
