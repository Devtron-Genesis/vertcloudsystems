...
// Theme init
if (!function_exists('invetex_widget_recent_reviews_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_widget_recent_reviews_theme_setup', 1 );
	function invetex_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('invetex_exists_visual_composer') && invetex_exists_visual_composer())
			add_action('invetex_action_shortcodes_list_vc','invetex_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('invetex_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'invetex_widget_recent_reviews_load' );
	function invetex_widget_recent_reviews_load() {
		register_widget('invetex_widget_recent_reviews');
	}
}

// Widget Class
class invetex_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'trx_utils'));
		parent::__construct( 'invetex_widget_recent_reviews', esc_html__('Themerex - Recent reviews', 'trx_utils'), $widget_ops );
...
*/
if ( !function_exists( 'invetex_sc_widget_recent_reviews' ) ) {
	function invetex_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'invetex_widget_recent_reviews';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, invetex_prepare_widgets_args(invetex_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('invetex_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	invetex_require_shortcode("trx_widget_recent_reviews", "invetex_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('invetex_widget_recent_reviews_reg_shortcodes_vc')) {
	function invetex_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'trx_utils'),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", 'trx_utils') ),
