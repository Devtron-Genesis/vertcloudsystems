...
// Theme init
if (!function_exists('invetex_widget_popular_posts_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_widget_popular_posts_theme_setup', 1 );
	function invetex_widget_popular_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('invetex_exists_visual_composer') && invetex_exists_visual_composer())
			add_action('invetex_action_shortcodes_list_vc','invetex_widget_popular_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('invetex_widget_popular_posts_load')) {
	add_action( 'widgets_init', 'invetex_widget_popular_posts_load' );
	function invetex_widget_popular_posts_load() {
		register_widget('invetex_widget_popular_posts');
	}
}

// Widget Class
class invetex_widget_popular_posts extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'invetex_widget_popular_posts', esc_html__('Themerex - Most Popular & Commented Posts', 'trx_utils'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? invetex_get_theme_option('blog_counters') : '';
		...
		for ($i=0; $i<3; $i++) {
			...
			if ($i==0) {			// Most popular
				$args['meta_key'] = invetex_storage_get('options_prefix').'_post_views_count';
				$args['orderby'] = 'meta_value_num';
				$show_counters = $show_counters ? 'views' : '';
			} else if ($i==2) {		// Most liked
				$args['meta_key'] = invetex_storage_get('options_prefix').'_post_likes_count';
				$args['orderby'] = 'meta_value_num';
...
*/
if ( !function_exists( 'invetex_sc_widget_popular_posts' ) ) {
	function invetex_sc_widget_popular_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'invetex_widget_popular_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_popular_posts' 
								. (invetex_exists_visual_composer() ? ' vc_widget_popular_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, invetex_prepare_widgets_args(invetex_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_popular_posts', 'widget_popular_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('invetex_shortcode_output', $output, 'trx_widget_popular_posts', $atts, $content);
	}
	invetex_require_shortcode("trx_widget_popular_posts", "invetex_sc_widget_popular_posts");
}


// Add [trx_widget_popular_posts] in the VC shortcodes list
if (!function_exists('invetex_widget_popular_posts_reg_shortcodes_vc')) {
	//add_action('invetex_action_shortcodes_list_vc','invetex_widget_popular_posts_reg_shortcodes_vc');
	function invetex_widget_popular_posts_reg_shortcodes_vc() {
