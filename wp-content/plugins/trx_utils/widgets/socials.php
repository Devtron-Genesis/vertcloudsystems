...
// Theme init
if (!function_exists('invetex_widget_socials_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_widget_socials_theme_setup', 1 );
	function invetex_widget_socials_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('invetex_exists_visual_composer') && invetex_exists_visual_composer())
			add_action('invetex_action_shortcodes_list_vc','invetex_widget_socials_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('invetex_widget_socials_load')) {
	add_action( 'widgets_init', 'invetex_widget_socials_load' );
	function invetex_widget_socials_load() {
		register_widget( 'invetex_widget_socials' );
	}
}

// Widget Class
class invetex_widget_socials extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'trx_utils') );
		parent::__construct( 'invetex_widget_socials', esc_html__('Themerex - Show logo and social links', 'trx_utils'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? invetex_do_shortcode($instance['text']) : '';
		...
		if ($title) invetex_show_layout($before_title . $title . $after_title);
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					if ($logo_text=='')		$logo_text = false;
					if ($logo_slogan=='')	$logo_slogan = false;
					if ($logo_image || $logo_text)
						invetex_show_logo($logo_image, false, false, false, $logo_text, $logo_slogan);
				}
				...
				if ($show_icons) {
					invetex_show_layout(invetex_sc_socials(array('size' => "tiny", 'shape' => 'round')));
				}
			?>
		</div>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_html_e('Description:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php echo wp_kses_data( __('Logo image:<br />(if empty - use logo from Theme Options)', 'trx_utils') ); ?></label>
            ...
            <?php
            invetex_show_layout(invetex_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php esc_html_e('Logo text:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_slogan' )); ?>"><?php esc_html_e('Logo slogan:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show logo:', 'trx_utils'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show', 'trx_utils'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php esc_html_e('Hide', 'trx_utils'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show social icons:', 'trx_utils'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show', 'trx_utils'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php esc_html_e('Hide', 'trx_utils'); ?></label>
		</p>
...
*/
if ( !function_exists( 'invetex_sc_widget_socials' ) ) {
	function invetex_sc_widget_socials($atts, $content=null){	
		...
		extract($atts);
		$type = 'invetex_widget_socials';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_socials' 
								. (invetex_exists_visual_composer() ? ' vc_widget_socials wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, invetex_prepare_widgets_args(invetex_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_socials', 'widget_socials') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('invetex_shortcode_output', $output, 'trx_widget_socials', $atts, $content);
	}
	invetex_require_shortcode("trx_widget_socials", "invetex_sc_widget_socials");
}


// Add [trx_widget_socials] in the VC shortcodes list
if (!function_exists('invetex_widget_socials_reg_shortcodes_vc')) {
	function invetex_widget_socials_reg_shortcodes_vc() {
