...
// Theme init
if (!function_exists('invetex_widget_recent_news_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_widget_recent_news_theme_setup', 1 );
	function invetex_widget_recent_news_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('invetex_exists_visual_composer') && invetex_exists_visual_composer())
			add_action('invetex_action_shortcodes_list_vc','invetex_widget_recent_news_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('invetex_widget_recent_news_load')) {
	add_action( 'widgets_init', 'invetex_widget_recent_news_load' );
	function invetex_widget_recent_news_load() {
		register_widget('invetex_widget_recent_news');
	}
}


// Widget Class
class invetex_widget_recent_news extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_news', 'description' => esc_html__('Show recent news in many styles', 'trx_utils'));
		parent::__construct( 'invetex_widget_recent_news', esc_html__('Themerex - Recent News', 'trx_utils'), $widget_ops );
	}
	...
	function widget($args, $instance) {
		...
		$output = invetex_sc_recent_news( array(
			'from_widget'		=> true,
		...
		if (!empty($output)) {
			...
			if ($widget_title) invetex_show_layout($before_title . $widget_title . $after_title);
	...
	function form($instance) {
		...
		$posts_types = invetex_get_list_posts_types(false);
		$categories = invetex_get_list_terms(false, invetex_get_taxonomy_categories_by_post_type($post_type));
		$news_styles = invetex_get_list_templates('news');
		?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('count')); ?>"><?php esc_html_e('Number of posts to be displayed (9 max):', 'trx_utils'); ?></label>
...
*/
if ( !function_exists( 'invetex_sc_recent_news' ) ) {
	function invetex_sc_recent_news($atts, $content=null){	
		...
		// Get categories list
		if (invetex_param_is_on($show_categories)) {
			if ( !invetex_storage_isset('categories_'.$category) ) {
				invetex_storage_set('categories_'.$category, get_categories(array(
					'orderby' => 'name',
					'parent' => $category
					))
				);
		...
		if (empty($from_widget)) {
			$widget_args = invetex_prepare_widgets_args(invetex_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_news', 'widget_recent_news');
			$output .= '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_recent_news_wrap' 
								. (invetex_exists_visual_composer() ? ' vc_recent_news wpb_content_element' : '') 
		...
		// Header
		if ( !empty($title) || !empty($subtitle) || (invetex_param_is_on($show_categories) && !invetex_storage_empty('categories_'.$category)) ) {
			$output	.= '<div class="sc_recent_news_header'.(invetex_param_is_on($show_categories) && !invetex_storage_empty('categories_'.$category) ? ' sc_recent_news_header_split' : '').'">'
			...
			// Categories list
			$cat_list = invetex_storage_get('categories_'.$category);
			if (invetex_param_is_on($show_categories) && !empty($cat_list)) {
		...
		$args = invetex_query_add_posts_and_cats($args, $ids, $post_type, $category, $taxonomy);
		...
		return apply_filters('invetex_shortcode_output', $output, 'trx_recent_news', $atts, $content);
	}
	invetex_require_shortcode("trx_recent_news", "invetex_sc_recent_news");
}


// Add [trx_recent_news] in the VC shortcodes list
if (!function_exists('invetex_widget_recent_news_reg_shortcodes_vc')) {
	//add_action('invetex_action_shortcodes_list_vc','invetex_widget_recent_news_reg_shortcodes_vc');
	function invetex_widget_recent_news_reg_shortcodes_vc() {
		
		$posts_types = invetex_get_list_posts_types(false);
		$categories = invetex_get_list_terms(false, invetex_get_taxonomy_categories_by_post_type('post'));
		$news_styles = invetex_get_list_templates('news');
		...
		vc_map( array(
				...
				"params" => array(
					...
					array(
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(invetex_array_merge(array(0 => esc_html__('- Select category -', 'trx_utils')), $categories)),
						"type" => "dropdown"
					),
					array(
						"param_name" => "count",
						"heading" => esc_html__("Total posts", 'trx_utils'),
						"description" => wp_kses_data( __("How many posts will be displayed? If used IDs - this parameter ignored. (9 max)", 'trx_utils') ),
						"admin_label" => true,
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => "3",
					...
					array(
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(invetex_get_list_sortings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "order",
						...
						"group" => esc_html__('Query', 'trx_utils'),
						"class" => "",
						"value" => array_flip(invetex_get_list_orderings()),
						"type" => "dropdown"
					),
					array(
						"param_name" => "ids",
