...
// Theme init
if (!function_exists('invetex_widget_top10_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_widget_top10_theme_setup', 1 );
	function invetex_widget_top10_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('invetex_exists_visual_composer') && invetex_exists_visual_composer())
			add_action('invetex_action_shortcodes_list_vc','invetex_widget_top10_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('invetex_widget_top10_load')) {
	add_action( 'widgets_init', 'invetex_widget_top10_load' );
	function invetex_widget_top10_load() {
		register_widget('invetex_widget_top10');
	}
}

// Widget Class
class invetex_widget_top10 extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( 'invetex_widget_top10', esc_html__('Themerex - Top 10 Posts', 'trx_utils'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<2; $i++) {
			...
			$post_rating = invetex_storage_get('options_prefix').'_reviews_avg'.($i==0 ? '' : '2');
...
*/
if ( !function_exists( 'invetex_sc_widget_top10' ) ) {
	function invetex_sc_widget_top10($atts, $content=null){	
		...
		extract($atts);
		$type = 'invetex_widget_top10';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_top10' 
								. (invetex_exists_visual_composer() ? ' vc_widget_top10 wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, invetex_prepare_widgets_args(invetex_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_top10', 'widget_top10') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('invetex_shortcode_output', $output, 'trx_widget_top10', $atts, $content);
	}
	invetex_require_shortcode("trx_widget_top10", "invetex_sc_widget_top10");
}


// Add [trx_widget_top10] in the VC shortcodes list
if (!function_exists('invetex_widget_top10_reg_shortcodes_vc')) {
	//add_action('invetex_action_shortcodes_list_vc','invetex_widget_top10_reg_shortcodes_vc');
	function invetex_widget_top10_reg_shortcodes_vc() {
