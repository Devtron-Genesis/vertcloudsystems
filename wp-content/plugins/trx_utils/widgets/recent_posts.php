...
// Theme init
if (!function_exists('invetex_widget_recent_posts_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_widget_recent_posts_theme_setup', 1 );
	function invetex_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('invetex_exists_visual_composer') && invetex_exists_visual_composer())
			add_action('invetex_action_shortcodes_list_vc','invetex_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('invetex_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'invetex_widget_recent_posts_load' );
	function invetex_widget_recent_posts_load() {
		register_widget('invetex_widget_recent_posts');
	}
}

// Widget Class
class invetex_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'trx_utils'));
		parent::__construct( 'invetex_widget_recent_posts', esc_html__('Themerex - Recent Posts', 'trx_utils'), $widget_ops );

		// Add thumb sizes into list
		invetex_add_thumb_sizes( array( 'layout' => 'widgets', 'w' => 75, 'h' => 75, 'title'=>esc_html__('Widgets', 'trx_utils') ) );
	}
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'trx_utils')), invetex_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'invetex_sc_widget_recent_posts' ) ) {
	function invetex_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'invetex_widget_recent_posts';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_posts' 
								. (invetex_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, invetex_prepare_widgets_args(invetex_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('invetex_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	invetex_require_shortcode("trx_widget_recent_posts", "invetex_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('invetex_widget_recent_posts_reg_shortcodes_vc')) {
	function invetex_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'trx_utils'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'trx_utils') ),
