...
if (!function_exists('invetex_action_theme_styles_no_less_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_action_theme_styles_no_less_theme_setup', 1 );
	function invetex_action_theme_styles_no_less_theme_setup() {

		// If no LESS support
		if (invetex_get_theme_setting('less_compiler') == 'no') {
			// Add theme styles
			add_action('invetex_action_add_styles', 				'invetex_action_theme_styles_no_less_add_styles');

			// Add colors and fonts for the WP Customizer
			add_action( 'customize_controls_print_footer_scripts',	'invetex_customizer_wp_css_template' );
			add_action( 'invetex_filter_get_css',					'invetex_customizer_wp_add_scheme_in_css', 100, 4 );
		}
	}
}
...
if (!function_exists('invetex_action_theme_styles_no_less_add_styles')) {
	function invetex_action_theme_styles_no_less_add_styles() {
		// Add stylesheet files
		if ( !is_customize_preview() && !isset($_GET['color_scheme']) && invetex_param_is_off(invetex_get_theme_option('debug_mode')) ) {
            wp_enqueue_style( 'invetex-theme-style', invetex_get_file_url('css/theme.css'), array(), null );
			wp_add_inline_style( 'invetex-theme-style', invetex_get_inline_css() );
		} else {
			wp_add_inline_style( 'invetex-main-style', invetex_get_custom_css() . invetex_get_inline_css() );
		}
	}
}
...
if (!function_exists('invetex_customizer_wp_add_scheme_in_css')) {
	function invetex_customizer_wp_add_scheme_in_css($css, $colors, $fonts, $scheme) {
		$rez = '';
		$in_comment = $in_rule = false;
		$allow = true;
		$scheme_class = '.scheme_' . trim($scheme) . ' ';
		$self_class = '.scheme_self';
		$self_class_len = invetex_strlen($self_class);
		...
		for ($i=0; $i<strlen($css_str); $i++) {
			...
			if ($in_comment) {
				...
				if ($ch=='/' && $css_str[$i+1]=='*') {
					...
					$rez .= $ch;
					$allow = true;
				} else if (invetex_strpos(" \t\r\n", $ch)===false) {
					if ($allow && invetex_substr($css_str, $i, $self_class_len) == $self_class) {
...
if ( !function_exists( 'invetex_customizer_wp_css_template' ) ) {
	function invetex_customizer_wp_css_template() {
		...
		$schemes = array_keys(invetex_get_list_color_schemes());
		if (count($schemes) > 0) {
			foreach ($schemes as $scheme) {
				echo '<'.trim($tmpl_holder).' type="text/html" id="tmpl-invetex-color-scheme-'.esc_attr($scheme).'">'
						. trim(invetex_get_custom_css( $colors, false, false, $scheme ))
					. '</'.trim($tmpl_holder).'>';
			}
		}
		...
		if (is_array($custom_fonts) && count($custom_fonts) > 0) {
			...
			echo '<'.trim($tmpl_holder).' type="text/html" id="tmpl-invetex-fonts">'
					. trim(invetex_get_custom_css( false, $fonts, false, false ))
				. '</'.trim($tmpl_holder).'>';
		}
...
if (!function_exists('invetex_customizer_wp_add_theme_colors')) {
	function invetex_customizer_wp_add_theme_colors($colors) {
		if (invetex_substr($colors['text'], 0, 2) == '{{') {
			...
			$colors['text_dark_0_05']	= invetex_hex2rgba( $colors['text_dark'], 0.05 );
			$colors['text_dark_0_1']	= invetex_hex2rgba( $colors['text_dark'], 0.1 );
			$colors['text_dark_0_8']	= invetex_hex2rgba( $colors['text_dark'], 0.8 );
			$colors['text_link_0_3']	= invetex_hex2rgba( $colors['text_link'], 0.3 );
			$colors['text_link_0_6']	= invetex_hex2rgba( $colors['text_link'], 0.6 );
			$colors['text_link_0_8']	= invetex_hex2rgba( $colors['text_link'], 0.8 );
			$colors['text_hover_0_2']	= invetex_hex2rgba( $colors['text_hover'], 0.2 );
			$colors['text_hover_0_3']	= invetex_hex2rgba( $colors['text_hover'], 0.3 );
			$colors['text_hover_0_8']	= invetex_hex2rgba( $colors['text_hover'], 0.8 );
			$colors['inverse_text_0_1']	= invetex_hex2rgba( $colors['inverse_text'], 0.1 );
			$colors['bg_color_0_8']		= invetex_hex2rgba( $colors['bg_color'], 0.8 );
			$colors['alter_text_0_1']	= invetex_hex2rgba( $colors['alter_text'], 0.1 );
			$colors['alter_bg_color_0_8'] = invetex_hex2rgba( $colors['alter_bg_color'], 0.8 );
			$colors['alter_bg_hover_0_5'] = invetex_hex2rgba( $colors['alter_bg_hover'], 0.5 );
			$colors['alter_bd_color_0_1'] = invetex_hex2rgba( $colors['alter_bd_color'], 0.1 );
		}
		return $colors;
	}
}
...
if (!function_exists('invetex_customizer_wp_add_theme_fonts')) {
	function invetex_customizer_wp_add_theme_fonts($fonts) {
		if (invetex_substr($fonts['h1_font-family'], 0, 2) == '{{') {
			...
			$fonts['logo_margin-top_0_75']	 = invetex_multiply_css_value( $fonts['logo_margin-top_value'], 0.75 );
			$fonts['logo_margin-bottom_0_5'] = invetex_multiply_css_value( $fonts['logo_margin-bottom_value'], 0.5 );
			$fonts['menu_margin-top_1']		 = $fonts['menu_margin-top_value'];
			$fonts['menu_margin-top_0_3']	 = invetex_multiply_css_value( $fonts['menu_margin-top_value'], 0.3 );
			$fonts['menu_margin-top_0_6']	 = invetex_multiply_css_value( $fonts['menu_margin-top_value'], 0.6 );
			$fonts['menu_margin-top_0_65']	 = invetex_multiply_css_value( $fonts['menu_margin-top_value'], 0.65 );
			$fonts['menu_margin-top_0_7']	 = invetex_multiply_css_value( $fonts['menu_margin-top_value'], 0.7 );
			$fonts['menu_margin-top_0_8']	 = invetex_multiply_css_value( $fonts['menu_margin-top_value'], 0.8 );
			$fonts['menu_margin-bottom_1']	 = $fonts['menu_margin-bottom_value'];
			$fonts['menu_margin-bottom_0_5'] = invetex_multiply_css_value( $fonts['menu_margin-bottom_value'], 0.5 );
			$fonts['menu_margin-bottom_0_6'] = invetex_multiply_css_value( $fonts['menu_margin-bottom_value'], 0.6 );
			$fonts['menu_height_1']			 = invetex_summ_css_value( invetex_summ_css_value( $fonts['menu_margin-top_value'],  $fonts['menu_margin-bottom_value'] ), ($fonts['menu_line-height_value']) );
...
if (!function_exists('invetex_get_custom_css')) {

	function invetex_get_custom_css($colors=null, $fonts=null, $minify=true, $only_scheme='') {

		$add_comment = $colors===null && $fonts===null && empty($only_scheme)
						? '/* ' . strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'invetex') ) . "*/\n"
						: '';
		...
		if ($fonts===null) {
			...
			$fonts = array();
			foreach ($custom_fonts as $tag => $font) {
				$fonts[$tag.'_font-family'] = !empty($font['font-family']) && !invetex_is_inherit_option($font['font-family'])
												? "font-family:\"" . esc_attr($font['font-family']) . '"' 
													. (isset($fonts_list[$font['font-family']]['family']) ? ',' . $fonts_list[$font['font-family']]['family'] : '' ) 
													. ";"
												: '';
				$fonts[$tag.'_font-size'] = !empty($font['font-size']) && !invetex_is_inherit_option($font['font-size'])
												? "font-size:" . invetex_prepare_css_value($font['font-size']) . ";"
												: '';
				$fonts[$tag.'_line-height'] = !empty($font['line-height']) && !invetex_is_inherit_option($font['line-height'])
												? "line-height: " . invetex_prepare_css_value($font['line-height']) . ";"
												: '';
				$fonts[$tag.'_line-height_value'] = !empty($font['line-height'])	// && !invetex_is_inherit_option($font['line-height'])
												? invetex_prepare_css_value($font['line-height'])
												: 'inherit';
				$fonts[$tag.'_font-weight'] = !empty($font['font-weight']) && !invetex_is_inherit_option($font['font-weight'])
												? "font-weight: " . trim($font['font-weight']) . ";"
												: '';
				$fonts[$tag.'_font-style'] = !empty($font['font-style']) && !invetex_is_inherit_option($font['font-style']) && invetex_strpos($font['font-style'], 'i')!==false
												? "font-style: italic;"
												: '';
				$fonts[$tag.'_text-decoration'] = !empty($font['font-style']) && !invetex_is_inherit_option($font['font-style']) && invetex_strpos($font['font-style'], 'u')!==false
												? "text-decoration: underline;"
												: '';
				$fonts[$tag.'_margin-top'] = !empty($font['margin-top']) && !invetex_is_inherit_option($font['margin-top'])
												? "margin-top: " . invetex_prepare_css_value($font['margin-top']) . ";"
												: '';
				$fonts[$tag.'_margin-top_value'] = !empty($font['margin-top'])	// && !invetex_is_inherit_option($font['margin-top'])
												? invetex_prepare_css_value($font['margin-top'])
												: 'inherit';
				$fonts[$tag.'_margin-bottom'] = !empty($font['margin-bottom']) && !invetex_is_inherit_option($font['margin-bottom'])
												? "margin-bottom: " . invetex_prepare_css_value($font['margin-bottom']) . ";"
												: '';
				$fonts[$tag.'_margin-bottom_value'] = !empty($font['margin-bottom'])	// && !invetex_is_inherit_option($font['margin-bottom'])
												? invetex_prepare_css_value($font['margin-bottom'])
												: 'inherit';
			}
		}
		if ($fonts) {
			$fonts = invetex_customizer_wp_add_theme_fonts($fonts);
...
.top_panel_middle .menu_main_wrap {
	margin-top: {$fonts['menu_margin-top_1']};
	margin-bottom: {$fonts['menu_margin-bottom_1']};
}
...
.top_panel_el {
	margin-top:{$fonts['menu_margin-top_0_8']};
	margin-bottom:{$fonts['menu_margin-bottom_0_5']};
}
...
FONTS;
		...
		if ($colors!==false) {
			$schemes = empty($only_scheme) ? array_keys(invetex_get_list_color_schemes()) : array($only_scheme);
			if (count($schemes) > 0) {
				$step = 1;
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = invetex_get_scheme_colors($scheme);

					// Make theme-specific colors and tints
					$colors = invetex_customizer_wp_add_theme_colors($colors);
...
.menu_pushy_wrap .menu_pushy_button {
	color: {$colors['text_link']};
}
...
.top_panel_style_3.boxed_style .top_panel_icon .contact_icon, 
.top_panel_style_3.boxed_style .menu_pushy_wrap .menu_pushy_button,
.top_panel_style_3.boxed_style .menu_user_nav > li > a,
.top_panel_style_3.boxed_style .top_panel_top,

.top_panel_over:not(.top_panel_fixed) .top_panel_icon .contact_icon, 
.top_panel_over:not(.top_panel_fixed) .top_panel_icon .search_submit,
.top_panel_over:not(.top_panel_fixed) .top_panel_top_socials .sc_socials.sc_socials_type_icons a,
.top_panel_over:not(.top_panel_fixed) .menu_pushy_wrap .menu_pushy_button,
.top_panel_over:not(.top_panel_fixed) .menu_user_nav > li > a,
.top_panel_over:not(.top_panel_fixed) .top_panel_top,
...
.top_panel_style_3.boxed_style .top_panel_icon .search_submit:hover,
.top_panel_style_3.boxed_style .top_panel_top_socials .sc_socials.sc_socials_type_icons a:hover,
.top_panel_style_3.boxed_style .menu_pushy_wrap .menu_pushy_button:hover,
.top_panel_style_3.boxed_style .menu_user_nav > li > a:hover,

.top_panel_over:not(.top_panel_fixed) .breadcrumbs a:hover,
.top_panel_over:not(.top_panel_fixed) .top_panel_icon .search_submit:hover,
.top_panel_over:not(.top_panel_fixed) .top_panel_top_socials .sc_socials.sc_socials_type_icons a:hover,
.top_panel_over:not(.top_panel_fixed) .menu_pushy_wrap .menu_pushy_button:hover,
.top_panel_over:not(.top_panel_fixed) .menu_user_nav > li > a:hover {
	color: {$colors['text_hover']};
}
...
/* Main menu */
.top_panel_style_3.boxed_style .top_panel_icon .search_submit,
.top_panel_style_3.boxed_style .top_panel_top_socials .sc_socials.sc_socials_type_icons a,
.top_panel_style_3.boxed_style .menu_main_nav > li > a,
.menu_main_nav > li > a {
...
.top_panel_style_3.boxed_style .menu_main_nav > li > a:hover,
...
.top_panel_style_7 .menu_main_nav > a:hover,
.top_panel_style_7 .menu_main_nav > li > a:hover,
.top_panel_style_7 .menu_main_nav > li.sfHover > a,
.top_panel_style_7 .menu_main_nav > li.current-menu-item > a,
.top_panel_style_7 .menu_main_nav > li.current-menu-parent > a,
.top_panel_style_7 .menu_main_nav > li.current-menu-ancestor > a {
	color: {$colors['alter_hover']};
}
...
.top_panel_middle .search_field:focus,
.top_panel_middle .search_submit:focus,
.top_panel_middle .search_submit:hover {

}
.top_panel_middle .search_state_opened .search_field,
.top_panel_middle .search_field:focus {
	color: {$colors['alter_dark']} !important;
	background-color: {$colors['alter_bg_hover']} !important;
}
.top_panel_wrap .search_style_fullscreen .search_field {
	{$fonts['p_font-family']}
}
...
.search_results a,
.search_results .post_info a,
.search_results .post_info a > span,
.search_results .post_more,
.search_results .search_results_close {
	color: {$colors['inverse_link']};
}
.search_results a:hover,
.search_results .post_info a:hover,
.search_results .post_info a:hover > span,
.search_results .post_more:hover,
.search_results .search_results_close:hover {
	color: {$colors['text_hover']};
}
...
.top_panel_over .header_mobile_style_3.boxed_style .menu_button,
.top_panel_over .header_mobile_style_3.boxed_style .menu_main_cart .top_panel_cart_button .contact_icon {
	color: {$colors['text_dark']};
}
.top_panel_over .header_mobile_style_3.boxed_style .menu_button:hover,
...
.single_post_nav .post_nav .post_nav_item:hover .post_nav_arrow:before {
	color: {$colors['text_hover']};
}
...
.post_des .readmore:hover,
.post_item .post_descr .readmore:hover {
	color: {$colors['text_hover']} !important;
}
.column-1_4 .sc_section .sc_item_button .sc_button:hover:after,
.post_des .readmore:hover:after,
.post_item .post_descr .readmore:hover:after {
...
.esg-navigationbutton:hover:before {
	color: {$colors['text_hover']} !important;
}
...
.cat_post_info .category_link:hover {
...
.widget_area_inner .post_info a:hover > span {
	color: {$colors['text_hover']};
}
...
.woocommerce .widget_area a.button, .woocommerce .widget_area button.button, .woocommerce .widget_area input.button,
.woocommerce .widget_area ul.products li.product .add_to_cart_button, .woocommerce-page .widget_area ul.products li.product .add_to_cart_button {
	background-color: {$colors['text_link']};
	color: {$colors['inverse_text']};
}

.woocommerce .widget_area a.button:hover, .woocommerce .widget_area button.button:hover, .woocommerce .widget_area input.button:hover,
.woocommerce .widget_area ul.products li.product .add_to_cart_button:hover, .woocommerce-page .widget_area ul.products li.product .add_to_cart_button:hover {
...
.sc_button.sc_button_style_filled2:not([class*="sc_button_hover_"]):hover {
	background-color: {$colors['text_link_0_8']};
	color: {$colors['inverse_text']};
}
...
[class*="sc_button_hover_fade"]:hover {
	background-color: {$colors['text_hover_0_8']} !important;
	color: {$colors['inverse_text']} !important;
}
.sc_button.sc_button_style_filled2.sc_button_hover_fade:hover {
...
.sc_button.sc_button_style_filled2.sc_button_hover_slide {
	background-color: {$colors['text_link']};
	color: {$colors['inverse_text']} !important;
}
/*
...
.sc_button.sc_button_style_filled2.sc_button_hover_slide:hover {
	background-color: {$colors['text_link_0_8']} !important;
}
*/
.sc_button_hover_slide_left {
    background: linear-gradient(to right, {$colors['text_hover_0_8']} 50%, {$colors['text_hover']} 50%) repeat scroll right bottom / 210% 100% rgba(0, 0, 0, 0) !important;
}
.sc_button.sc_button_style_filled2.sc_button_hover_slide_left {
	background: linear-gradient(to right, {$colors['text_link_0_8']} 50%, {$colors['text_link']} 50%) repeat scroll right bottom / 210% 100% rgba(0, 0, 0, 0) !important;
}
.sc_button_hover_slide_top {
    background: linear-gradient(to bottom, {$colors['text_hover_0_8']} 50%, {$colors['text_hover']} 50%) repeat scroll right bottom / 100% 210% rgba(0, 0, 0, 0) !important;
}
.sc_button.sc_button_style_filled2.sc_button_hover_slide_top {
	background: linear-gradient(to bottom, {$colors['text_link_0_8']} 50%, {$colors['text_link']} 50%) repeat scroll right bottom / 100% 210% rgba(0, 0, 0, 0) !important;
}
...
.sc_price_wrap:hover .sc_price_block.sc_price_block_style_1 {
	background-color: {$colors['alter_bg_color']};
}
...
.sc_skills_counter .sc_skills_item.sc_skills_style_4 .sc_skills_info_2 {
	{$fonts['p_font-family']}
}
...
.scheme_original_in .sc_testimonial_author_position {
	{$fonts['info_font-family']}
}
...
CSS;
					...
					$rez = apply_filters('invetex_filter_get_css', $rez, $colors, $fonts, $scheme);
		...
		if (!empty($css_str))
			$css_str = $add_comment . ($minify ? invetex_minify_css($css_str) : $css_str);
