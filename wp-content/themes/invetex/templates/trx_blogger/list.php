...
if ( !function_exists( 'invetex_template_list_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_list_theme_setup', 1 );
	function invetex_template_list_theme_setup() {
		invetex_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'invetex')
			));
	}
}

// Template output
if ( !function_exists( 'invetex_template_list_output' ) ) {
	function invetex_template_list_output($post_options, $post_data) {
