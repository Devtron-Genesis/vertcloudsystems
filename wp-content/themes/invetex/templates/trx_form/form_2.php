...
if ( !function_exists( 'invetex_template_form_2_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_form_2_theme_setup', 1 );
	function invetex_template_form_2_theme_setup() {
		invetex_add_template(array(
			'layout' => 'form_2',
			'mode'   => 'forms',
			'title'  => esc_html__('Contact Form 2', 'invetex')
			));
	}
}

// Template output
if ( !function_exists( 'invetex_template_form_2_output' ) ) {
	function invetex_template_form_2_output($post_options, $post_data) {
        ...
        global $INVETEX_STORAGE;
			...
			?><div class="sc_form_fields">
				<form <?php echo !empty($post_options['id']) ? ' id="'.esc_attr($post_options['id']).'_form"' : ''; ?> 
                    ...
                    ?>
					<div class="sc_form_item sc_form_button"><button class="sc_button sc_button_style_filled sc_button_size_medium sc_button_iconed icon-paper-plane-light"><?php esc_html_e('Leave a Reply', 'invetex'); ?></button></div>
