...
if ( !function_exists( 'invetex_template_form_3_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_form_3_theme_setup', 1 );
	function invetex_template_form_3_theme_setup() {
		invetex_add_template(array(
			'layout' => 'form_3',
			'mode'   => 'forms',
			'title'  => esc_html__('Contact Form 3', 'invetex')
			));
	}
}

// Template output
if ( !function_exists( 'invetex_template_form_3_output' ) ) {
	function invetex_template_form_3_output($post_options, $post_data) {
				...
				<form <?php echo !empty($post_options['id']) ? ' id="'.esc_attr($post_options['id']).'_form"' : ''; ?> 
					...
					<?php invetex_sc_form_show_fields($post_options['fields']); ?>
						...
						<div class="column-1_2">
							...
							<div class="sc_form_item sc_form_field label_over"><input class="sc_form_phone" type="text" name="phone"<?php if ($form_style=='default') echo ' placeholder="'.esc_attr__('Phone', 'invetex').'"'; ?> aria-required="true"><?php
								if ($form_style!='default') {
									...
									?><span class="sc_form_label_content" data-content="<?php esc_html_e('Phone', 'invetex'); ?>"><?php esc_html_e('Phone', 'invetex'); ?></span><?php
									?></label><?php
								}
								?></div>
						</div><div class="column-1_2">
                    ...
                    ?>
					<div class="sc_form_item sc_form_button"><button class="sc_button sc_button_style_filled sc_button_size_medium sc_button_iconed icon-paper-plane-light"><?php esc_html_e('Send Request', 'invetex'); ?></button></div>
