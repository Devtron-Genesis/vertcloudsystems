...
if ( !function_exists( 'invetex_template_attachment_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_attachment_theme_setup', 1 );
	function invetex_template_attachment_theme_setup() {
		invetex_add_template(array(
			'layout' => 'attachment',
			'mode'   => 'internal',
			'title'  => esc_html__('Attachment page layout', 'invetex'),
...
// Template output
if ( !function_exists( 'invetex_template_attachment_output' ) ) {
	function invetex_template_attachment_output($post_options, $post_data) {
		...
		<article <?php post_class('post_item post_item_attachment template_attachment'); ?>>
			...
			<div class="post_featured">
				<div class="post_thumb post_nav" data-image="<?php echo esc_url($post_data['post_attachment']); ?>" data-title="<?php echo esc_attr($post_data['post_title']); ?>">
					...
					if ( isset( $attachments[ $k-1 ] ) ) {
						...
						$desc = invetex_strshort(!empty($attachments[ $k-1 ]->post_excerpt) ? $attachments[ $k-1 ]->post_excerpt : $attachments[ $k-1 ]->post_title, 30);
						?>
						<a class="post_nav_item post_nav_prev" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Previous item', 'invetex'); ?></span>
								<span class="post_nav_info_description"><?php invetex_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
					}
					if ( isset( $attachments[ $k+1 ] ) ) {
						...
						<a class="post_nav_item post_nav_next" href="<?php echo esc_url($link); ?>">
							<span class="post_nav_info">
								<span class="post_nav_info_title"><?php esc_html_e('Next item', 'invetex'); ?></span>
								<span class="post_nav_info_description"><?php invetex_show_layout($desc); ?></span>
							</span>
						</a>
						<?php
			...
			<div class="post_content">
				...
				if ($post_data['post_protected']) {
					...
					echo !empty($post_data['post_content']) ? trim($post_data['post_content']) : esc_html__('No image description ...', 'invetex'); 
					wp_link_pages( array( 
						'before' => '<div class="nav_pages_parts"><span class="pages">' . esc_html__( 'Pages:', 'invetex' ) . '</span>', 
						'after' => '</div>',
