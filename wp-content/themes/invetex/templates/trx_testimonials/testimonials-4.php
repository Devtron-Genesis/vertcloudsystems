...
if ( !function_exists( 'invetex_template_testimonials_4_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_testimonials_4_theme_setup', 1 );
	function invetex_template_testimonials_4_theme_setup() {
		invetex_add_template(array(
			'layout' => 'testimonials-4',
			'template' => 'testimonials-4',
			'mode'   => 'testimonials',
			'title'  => esc_html__('Testimonials /Style 4/', 'invetex'),
...
// Template output
if ( !function_exists( 'invetex_template_testimonials_4_output' ) ) {
	function invetex_template_testimonials_4_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?> class="sc_testimonial_item<?php echo (!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php echo !empty($post_options['tag_css']) ? ' style="'.esc_attr($post_options['tag_css']).'"' : '';?>>
				...
				<div class="sc_testimonial_avatar"><?php invetex_show_layout($post_options['photo']); ?></div>
				<?php } ?>
				<div class="sc_testimonial_content"><?php invetex_show_layout($post_data['post_content']); ?></div>
