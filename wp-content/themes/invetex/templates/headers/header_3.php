...
if ( !function_exists( 'invetex_template_header_3_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_header_3_theme_setup', 1 );
	function invetex_template_header_3_theme_setup() {
		invetex_add_template(array(
			'layout' => 'header_3',
			'mode'   => 'header',
			'title'  => esc_html__('Header 3', 'invetex'),
			'icon'   => invetex_get_file_url('templates/headers/images/3.jpg')
			));
	}
}

// Template output
if ( !function_exists( 'invetex_template_header_3_output' ) ) {
	function invetex_template_header_3_output($post_options, $post_data) {
		...
		<header class="top_panel_wrap top_panel_style_3 scheme_<?php echo esc_attr($post_options['scheme']); ?>">
			...
			<div class="top_panel_middle" <?php invetex_show_layout($header_css); ?>>
				<div class="content_wrap">
					...
					<div class="menu_main_wrap">
						<nav class="menu_main_nav_area menu_hover_<?php echo esc_attr(invetex_get_theme_option('menu_hover')); ?>">
							...
							invetex_show_layout($menu_main);
							?>
						</nav>
						<?php
						if (invetex_get_custom_option('show_search')=='yes')
                            if(function_exists('invetex_sc_search'))
							invetex_show_layout(invetex_sc_search(array('class'=>"top_panel_el top_panel_icon", 'state'=>"closed", 'style'=>invetex_get_theme_option('search_style'))));
						?>
