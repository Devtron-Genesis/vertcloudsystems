...
if ( !function_exists( 'invetex_template_news_magazine_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_template_news_magazine_theme_setup', 1 );
	function invetex_template_news_magazine_theme_setup() {
		invetex_add_template(array(
			'layout' => 'news-magazine',
			'template' => 'news-magazine',
			'mode'   => 'news',
			'title'  => esc_html__('Recent News /Style Magazine/', 'invetex'),
...
// Template output
if ( !function_exists( 'invetex_template_news_magazine_output' ) ) {
	function invetex_template_news_magazine_output($post_options, $post_data) {
		...
		?><article id="post-<?php echo esc_html($post_data['post_id']); ?>" 
				...
				<div class="post_header entry-header">
					...
					if ($show_title) {
						if (!isset($post_options['links']) || $post_options['links']) {
							?>
							<<?php echo esc_html($title_tag); ?> class="post_title entry-title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php invetex_show_layout($post_data['post_title']); ?></a></<?php echo esc_html($title_tag); ?>>
							<?php
						} else {
							?>
							<<?php echo esc_html($title_tag); ?> class="post_title entry-title"><?php invetex_show_layout($post_data['post_title']); ?></<?php echo esc_html($title_tag); ?>>
							<?php
						}
					}
					
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						?><div class="post_meta"><span class="post_meta_author"><?php invetex_show_layout($post_data['post_author_link']); ?></span><?php
			...
			if ($number <= $featured) {
				...
				<div class="post_footer entry-footer">
					...
					<?php
					if ( in_array( $post_data['post_type'], array( 'post', 'attachment' ) ) ) {
						$post_options['counters'] = 'views,comments,edit,captions';		//invetex_get_theme_option('blog_counters');
