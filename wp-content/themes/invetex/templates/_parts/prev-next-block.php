<?php
// Get template args
extract(invetex_template_get_args('prev-next-block'));
...
if (!$post_data['post_protected']) {
	...
	if (!empty($post_options['dedicated'])) {
		...
		$page_style = invetex_get_custom_option('single_style');
		?>
		<div class="post_thumb post_nav"<?php echo 'single-portfolio-fullscreen'==$page_style ? ' style="background-image:url('.esc_url($post_data['post_attachment']).');"' : ''; ?>>
			<?php 
			if ($page_style!='single-portfolio-fullscreen') invetex_show_layout($post_data['post_thumb']);
			$cur = get_post();
			$args = array(
			...
			if ( $prev ) {
				...
				$desc = invetex_strshort($prev->post_title, 30);
				?>
			...
			if ( $next ) {
				...
				$desc = invetex_strshort($next->post_title, 30);
				?>
