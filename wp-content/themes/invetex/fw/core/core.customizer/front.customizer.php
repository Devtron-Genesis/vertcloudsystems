...
$color_scheme = invetex_get_custom_option('body_scheme');
if (empty($color_scheme)) $color_scheme = 'original';
$color_scheme_list = invetex_get_list_color_schemes();
$body_style = invetex_get_custom_option('body_style');
$bg_color 	= invetex_get_custom_option('bg_color');
$bg_pattern = invetex_get_custom_option('bg_pattern');
$bg_image 	= invetex_get_custom_option('bg_image');
...
<div id="custom_options" class="custom_options <?php echo esc_attr($co_style); ?>">
	...
	<div class="co_header">
		<div class="co_title">
			<span><?php esc_html_e('Style switcher', 'invetex'); ?></span>
			<a href="#" id="co_theme_reset" class="co_reset icon-retweet" title="<?php esc_attr_e('Reset to defaults', 'invetex'); ?>"><?php esc_html_e('RESET', 'invetex'); ?></a>
		</div>
	</div>
		...
		<div class="sc_scroll_wrapper swiper-wrapper">
			<div class="sc_scroll_slide swiper-slide">
				...
				<div class="co_section">
					<div class="co_label"><?php esc_html_e('Body styles', 'invetex'); ?></div>
					<div class="co_switch_box co_switch_horizontal co_switch_columns_2<?php /* co_switch_vertical|horizontal,co_switch_columns_3|4 */ ?>" data-options="body_style">
						...
						<a href="#" data-value="boxed"><?php esc_html_e('Boxed', 'invetex'); ?></a>
						<a href="#" data-value="wide"><?php esc_html_e('Wide', 'invetex'); ?></a>
					</div>
				</div>

				<div class="co_section">
					<div class="co_label"><?php esc_html_e('Color scheme', 'invetex'); ?></div>
					<div id="co_scheme_list" class="co_image_check" data-options="body_scheme">
						<?php 
						if (is_array($color_scheme_list) && count($color_scheme_list) > 0) {
							foreach ($color_scheme_list as $k=>$v) {
								$scheme = invetex_get_file_url('images/schemes/'.($k).'.jpg');
								?>
				...
				<div class="co_section">
					<div class="co_label"><?php esc_html_e('Background pattern', 'invetex'); ?></div>
					<div id="co_bg_pattern_list" class="co_image_check" data-options="bg_pattern">
						<?php
						for ($i=1; $i<=5; $i++) {
							$pattern = invetex_get_file_url('images/bg/pattern_'.intval($i).'.jpg');
							$thumb   = invetex_get_file_url('images/bg/pattern_'.intval($i).'_thumb.jpg');
							?>
				...
				<div class="co_section">
					<div class="co_label"><?php esc_html_e('Background image', 'invetex'); ?></div>
					<div id="co_bg_images_list" class="co_image_check" data-options="bg_image">
						<?php
						for ($i=1; $i<=3; $i++) {
							$image = invetex_get_file_url('images/bg/image_'.intval($i).'.jpg');
							$thumb = invetex_get_file_url('images/bg/image_'.intval($i).'_thumb.jpg');
							?>
