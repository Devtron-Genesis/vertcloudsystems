...
if ( !function_exists( 'invetex_core_customizer_theme_setup' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_core_customizer_theme_setup', 1 );
	function invetex_core_customizer_theme_setup() {

		// Load Color schemes then Theme Options are loaded
		add_action('invetex_action_load_main_options',					'invetex_core_customizer_load_options');

		// Recompile LESS and save CSS
		add_action('invetex_action_compile_less',						'invetex_core_customizer_compile_less');
		add_filter('invetex_filter_prepare_less',						'invetex_core_customizer_prepare_less');

		if ( is_admin() ) {
	
			// Ajax Save and Export Action handler
			add_action('wp_ajax_invetex_options_save', 				'invetex_core_customizer_save_options');
	
			// Ajax Delete color scheme Action handler
			add_action('wp_ajax_invetex_options_scheme_delete', 		'invetex_core_customizer_scheme_delete');

			// Ajax Copy color scheme Action handler
			add_action('wp_ajax_invetex_options_scheme_copy', 			'invetex_core_customizer_scheme_copy');
		}
		
	}
}

if ( !function_exists( 'invetex_core_customizer_theme_setup2' ) ) {
	add_action( 'invetex_action_before_init_theme', 'invetex_core_customizer_theme_setup2', 11 );
	function invetex_core_customizer_theme_setup2() {

		if ( is_admin() ) {

			// Add Theme Options in WP menu
			add_action('admin_menu', 								'invetex_core_customizer_admin_menu_item');
		}
...
if ( !function_exists( 'invetex_core_customizer_admin_menu_item' ) ) {
	function invetex_core_customizer_admin_menu_item() {
		invetex_admin_add_menu_item('theme', array(
			'page_title' => esc_html__('Fonts & Colors', 'invetex'),
			'menu_title' => esc_html__('Fonts & Colors', 'invetex'),
			'capability' => 'manage_options',
			'menu_slug'  => 'invetex_options_customizer',
			'callback'   => 'invetex_core_customizer_page',
			'icon'		 => ''
			)
		);
	}
...
if ( !function_exists( 'invetex_core_customizer_load_options' ) ) {
	function invetex_core_customizer_load_options() {
		$mode = isset($_POST['mode']) ? $_POST['mode'] : '';
		$override = isset($_POST['override']) ? $_POST['override'] : '';
		if ($mode!='reset' || $override!='customizer') {
			$storage = get_option( invetex_storage_get('options_prefix') . '_options_custom_colors' );
			if (!empty($storage)) invetex_storage_set('custom_colors', $storage);
			$storage = get_option( invetex_storage_get('options_prefix') . '_options_custom_fonts' );
			if (!empty($storage)) invetex_storage_set('custom_fonts', $storage);
		}
	}
}

// Ajax Save and Export Action handler
if ( !function_exists( 'invetex_core_customizer_save_options' ) ) {
	function invetex_core_customizer_save_options() {
		...
		$colors = invetex_storage_get('custom_colors');
		if ($mode == 'save') {
			if (is_array($colors) && count($colors) > 0) {
				$order = !empty($data['invetex_options_schemes_order']) ? explode(',', $data['invetex_options_schemes_order']) : array_keys($colors);
				$schemes = array();
				foreach ($order as $slug) {
					$new_slug = $data[$slug.'-slug'];
					if (empty($new_slug)) $new_slug = invetex_get_slug($scheme['title']);
				...
				$colors = apply_filters('invetex_filter_save_custom_colors', $schemes);
				invetex_storage_set('custom_colors', $colors);
				update_option( invetex_storage_get('options_prefix') . '_options_custom_colors', $colors);
			}
		} else if ($mode == 'reset') {
			delete_option( invetex_storage_get('options_prefix') . '_options_custom_colors');
		}
		...
		if ($mode == 'save') {
					...
					if (is_array($font) && count($font) > 0) {
						foreach ($font as $key=>$value) {
							if (isset($data[$slug.'-'.$key]))
								$fonts[$slug][$key] = invetex_is_inherit_option($data[$slug.'-'.$key]) ? '' : $data[$slug.'-'.$key];
						}
					}
				}
			...
			delete_option( invetex_storage_get('options_prefix') . '_options_custom_fonts');
		}
...
// Ajax Delete color scheme Action handler
if ( !function_exists( 'invetex_core_customizer_scheme_delete' ) ) {
	function invetex_core_customizer_scheme_delete() {
		...
		if (isset($colors[$scheme])) {
			if (count($colors) > 1) {
				...
				$response['error'] = sprintf(esc_html__('You cannot delete last color scheme!', 'invetex'), $scheme);
		} else
			$response['error'] = sprintf(esc_html__('Color Scheme %s not found!', 'invetex'), $scheme);
...
// Ajax Copy color scheme Action handler
if ( !function_exists( 'invetex_core_customizer_scheme_copy' ) ) {
	function invetex_core_customizer_scheme_copy() {
			...
			foreach ($order as $slug) {
				if (is_array($colors[$slug]) && count($colors[$slug]) > 0) {
					...
					if ($slug == $scheme) {
						...
						$schemes[$new_slug]['title'] .= ' '.esc_html__('(Copy)', 'invetex');
					}
				}
			}
			$schemes = apply_filters('invetex_filter_save_custom_colors', $schemes);
...
if (!function_exists('invetex_core_customizer_compile_less')) {
	function invetex_core_customizer_compile_less() {
		if (invetex_get_theme_setting('less_compiler')=='no') return;
		$files = array();
		if (file_exists(invetex_get_file_dir('css/_utils.less'))) 	$files[] = invetex_get_file_dir('css/_utils.less');
		$files = apply_filters('invetex_filter_compile_less', $files);
		if (count($files) > 0) invetex_compile_less($files);
	}
}
...
if ( !function_exists( 'invetex_core_customizer_page' ) ) {
	function invetex_core_customizer_page() {
		...
		if (is_array($colors) && count($colors) > 0) {
			...
			$demo_block = '';
			if (invetex_get_theme_setting('customizer_demo') && file_exists(trailingslashit(get_template_directory()) . INVETEX_FW_DIR . '/core/core.customizer/core.customizer.demo.php')) {
				ob_start();
				require_once trailingslashit( get_template_directory() ) . INVETEX_FW_DIR . '/core/core.customizer/core.customizer.demo.php';
				$demo_block = ob_get_contents();
				ob_end_clean();
			}
			$options["partition_schemes"] = array(
				"title" => esc_html__('Color schemes', 'invetex'),
				"override" => "customizer",
			...
			foreach ($colors as $slug=>$scheme) {
				...
				$options["{$slug}-description"] = array(
					"title" => sprintf(esc_html__('Color scheme "%s"', 'invetex'), $scheme['title']),
					"desc" => wp_kses_data( sprintf(__('Specify the color for each element in the scheme "%s". After that you will be able to use your color scheme for the entire page, any part thereof and/or for the shortcodes!', 'invetex'), $scheme['title']) ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-buttons_label"] = array(
					"desc" => wp_kses_data( __("You can duplicate current color scheme (appear on new tab) or delete it (if not last scheme)", 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "4_6 first",
					"type" => "label");
				...
				$options["{$slug}-button_delete"] = array(
					"title" => esc_html__('Delete',  'invetex'),
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_6 last",
				...
				if ($demo_block) {
					$options["{$slug}-demo"] = array(
						"title" => esc_html__('Usage demo', 'invetex'),
						"desc" => wp_kses_data( __('Below you can see the example of decoration of the page with selected colors.', 'invetex') )
									. trim($demo_block),
						"override" => "customizer",
						"type" => "info");
				}
...
if (isset($scheme['bg_color'])) {
				...
				$options["{$slug}-block_info"] = array(
					"title" => esc_html__('Page/Block decoration', 'invetex'),
					"desc" => wp_kses_data( __('Specify border and background to decorate whole page (if scheme accepted to the page) or entire block/section.', 'invetex') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-bd_color_label"] = array(
					...
					"desc" => wp_kses_data( __('Select the border color and it hover state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_color_label"] = array(
					...
					"desc" => wp_kses_data( __('Select the background color and it hover state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['bg_image'])) {
				...
				$options["{$slug}-bg_image_label"] = array(
					"title" => esc_html__('Background image', 'invetex'),
					"desc" => wp_kses_data( __('Select first background image and it display parameters', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-bg_image_repeat"] = array(
					"title" => esc_html__('Repeat', 'invetex'),
					"std" => "",
					"val" => $scheme['bg_image_repeat'],
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_5",
					"options" => invetex_get_list_bg_image_repeats(),
					"type" => "select");

				$options["{$slug}-bg_image_attachment"] = array(
					"title" => esc_html__('Attachment', 'invetex'),
					"std" => "",
					"val" => $scheme['bg_image_attachment'],
					"override" => "customizer",
					"divider" => false,
					"columns" => "1_5",
					"options" => invetex_get_list_bg_image_attachments(),
					"type" => "select");
	
				// Background image 2
				$options["{$slug}-bg_image2_label"] = array(
					"title" => esc_html__('Background image 2', 'invetex'),
					"desc" => wp_kses_data( __('Select second background image and it display parameters', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['accent2'])) {
				...
				$options["{$slug}-accent_info"] = array(
					"title" => esc_html__('Accented colors', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors for the accented areas in your site.', 'invetex') ),
					"override" => "customizer",
					"type" => "info");

				// Accent 2 color
				$options["{$slug}-accent2_label"] = array(
					"title" => esc_html__('Accent 2', 'invetex'),
					"desc" => wp_kses_data( __('Select color for accented elements and their hover state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['accent3'])) {
				...
				$options["{$slug}-accent3_label"] = array(
					"title" => esc_html__('Accent 3', 'invetex'),
					"desc" => wp_kses_data( __('Select color for accented elements and their hover state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
...
if (isset($scheme['text'])) {
				...
				$options["{$slug}-text_info"] = array(
					"title" => esc_html__('Text and Headers', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors for the plain text, post info blocks and headers', 'invetex') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-text_label"] = array(
					...
					"desc" => wp_kses_data( __('Select colors for the text: normal text color, light text (for example - post info) and dark text (headers, bold text, etc.)', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['text_link'])) {
				...
				$options["{$slug}-text_link_label"] = array(
					"title" => esc_html__('Text links', 'invetex'),
					"desc" => wp_kses_data( __('Select color for the links and their hover state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-text_hover"] = array(
					"title" => esc_html__('Hover', 'invetex'),
					"std" => "",
...
if (isset($scheme['inverse_text'])) {
				...
				$options["{$slug}-inverse_info"] = array(
					"title" => esc_html__('Inverse blocks', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors for the headers, plain text, links and post info blocks in the accented areas (with background color equal to text link)', 'invetex') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-inverse_label"] = array(
					...
					"desc" => wp_kses_data( __('Select colors for inversed text (text on accented background)', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-inverse_label2"] = array(
					"title" => esc_html__('Inverse links', 'invetex'),
					"desc" => wp_kses_data( __('Select colors for inversed links (links on accented background)', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['input_text'])) {
				...
				$options["{$slug}-input_info"] = array(
					"title" => esc_html__('Input colors: form fields and textareas', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors to decorate input fields in the forms', 'invetex') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-input_text_label"] = array(
					"title" => esc_html__('Text', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors for the input fields for all states: disabled, inactive, active', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-input_text"] = array(
					"title" => esc_html__('Inactive', 'invetex'),
					"std" => "",
				...
				$options["{$slug}-input_light"] = array(
					"title" => esc_html__('Disabled', 'invetex'),
					"std" => "",
				...
				$options["{$slug}-input_dark"] = array(
					"title" => esc_html__('Active', 'invetex'),
					"std" => "",
				...
				$options["{$slug}-input_bd_color_label"] = array(
					"title" => esc_html__('Border color', 'invetex'),
					"desc" => wp_kses_data( __('Select the border colors for the normal state and for active (focused) field', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-input_bg_color_label"] = array(
					"title" => esc_html__('Background Color', 'invetex'),
					"desc" => wp_kses_data( __('Select the background colors for the normal state and for active (focused) field', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['alter_text'])) {
				...
				$options["{$slug}-alter_info"] = array(
					"title" => esc_html__('Alternative colors: Highlighted areas, submenu items, etc.', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors to decorate highlighted blocks in the text, submenu items, etc.', 'invetex') ),
					"override" => "customizer",
					"type" => "info");
				...
				$options["{$slug}-alter_text_label"] = array(
					"title" => esc_html__('Text and Headers', 'invetex'),
					"desc" => wp_kses_data( __('Specify colors for the plain text, post info blocks and headers in the highlight blocks', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_link_label"] = array(
					...
					"desc" => wp_kses_data( __('Specify colors for the links in the highlight blocks', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bd_color_label"] = array(
					"title" => esc_html__('Border color', 'invetex'),
					"desc" => wp_kses_data( __('Select the border colors for the normal and hovered state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
				...
				$options["{$slug}-alter_bg_color_label"] = array(
					"title" => esc_html__('Background Color', 'invetex'),
					"desc" => wp_kses_data( __('Select the background colors for the normal and hovered state', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
...
if (isset($scheme['alter_bg_image'])) {
				...
				$options["{$slug}-alter_bg_image_label"] = array(
					"title" => esc_html__('Background image', 'invetex'),
					"desc" => wp_kses_data( __('Select alter background image and it display parameters', 'invetex') ),
					"override" => "customizer",
					"divider" => false,
					"columns" => "2_5 first",
					"type" => "label");
			...
			foreach ($fonts as $slug=>$font) {
				...
				if (isset($font['font-size'])) {
					$options["{$slug}-font-size"] = array(
						"title" => $show_titles ? esc_html__('Size', 'invetex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['line-height'])) {
					$options["{$slug}-line-height"] = array(
						"title" => $show_titles ? esc_html__('Line height', 'invetex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-weight'])) {
					$options["{$slug}-font-weight"] = array(
						"title" => $show_titles ? esc_html__('Weight', 'invetex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['font-style'])) {
					$options["{$slug}-font-style"] = array(
						"title" => $show_titles ? esc_html__('Style', 'invetex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-top'])) {
					$options["{$slug}-margin-top"] = array(
						"title" => $show_titles ? esc_html__('Margin Top', 'invetex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
				...
				if (isset($font['margin-bottom'])) {
					$options["{$slug}-margin-bottom"] = array(
						"title" => $show_titles ? esc_html__('Margin Bottom', 'invetex') : '',
						"desc" => '',
						"divider" => false,
						"columns" => "1_8",
						"std" => "",
		...
		invetex_core_customizer_load_scripts();
		// Prepare javascripts global variables
		invetex_core_customizer_prepare_scripts();
		
		// Build Options page
		invetex_options_page_start(array(
			'title' => esc_html__('Fonts & Colors', 'invetex'),
			"icon" => "iconadmin-cog",
			"subtitle" => esc_html__('Fonts settings & Color schemes', 'invetex'),
			"description" => wp_kses_data( __('Customize fonts and colors for your site.', 'invetex') ),
			'data' => $options,
			'create_form' => true,
		...
		if (is_array($options) && count($options) > 0) {
			foreach ($options as $id=>$option) { 
				invetex_options_show_field($id, $option);
			}
		}
...
if (!function_exists('invetex_core_customizer_prepare_less')) {
	function invetex_core_customizer_prepare_less() {

		// Prefix for override rules
		$prefix = invetex_get_theme_setting('less_prefix');
		// Use nested selectors: increase .css size, but allow use nested color schemes
		$nested = invetex_get_theme_setting('less_nested');

		$out = '';

		// Custom fonts
		$fonts_list = invetex_get_list_fonts(false);
		...
		foreach ($custom_fonts as $slug => $font) {
			...
			if (!empty($font['font-family']) && !invetex_is_inherit_option($font['font-family']))
			...
			if (!empty($font['font-size']) && !invetex_is_inherit_option($font['font-size']))
				$out .= "@{$slug}_fs: " . invetex_prepare_css_value($font['font-size']) . ";\n";
			else
				$out .= "@{$slug}_fs: inherit;\n";
			
			if (!empty($font['line-height']) && !invetex_is_inherit_option($font['line-height']))
				$out .= "@{$slug}_lh: " . invetex_prepare_css_value($font['line-height']) . ";\n";
			else
				$out .= "@{$slug}_lh: inherit;\n";

			if (!empty($font['font-weight']) && !invetex_is_inherit_option($font['font-weight']))
				$out .= "@{$slug}_fw: " . trim($font['font-weight']) . ";\n";
			else
				$out .= "@{$slug}_fw: inherit;\n";

			if (!empty($font['font-style']) && !invetex_is_inherit_option($font['font-style']) && invetex_strpos($font['font-style'], 'i')!==false)
				$out .= "@{$slug}_fl: italic;\n";
			else
				$out .= "@{$slug}_fl: inherit;\n";

			if (!empty($font['font-style']) && !invetex_is_inherit_option($font['font-style']) && invetex_strpos($font['font-style'], 'u')!==false)
				$out .= "@{$slug}_td: underline;\n";
			else
				$out .= "@{$slug}_td: inherit;\n";

			if (!empty($font['margin-top']) && !invetex_is_inherit_option($font['margin-top']))
				$out .= "@{$slug}_mt: " . invetex_prepare_css_value($font['margin-top']) . ";\n";
			else
				$out .= "@{$slug}_mt: inherit;\n";

			if (!empty($font['margin-bottom']) && !invetex_is_inherit_option($font['margin-bottom']))
				$out .= "@{$slug}_mb: " . invetex_prepare_css_value($font['margin-bottom']) . ";\n";
			else
			...
			if (!empty($font['font-size']) && !invetex_is_inherit_option($font['font-size']))
				$out .= "\tfont-size:" . invetex_prepare_css_value($font['font-size']) . ";\n";
			if (!empty($font['line-height']) && !invetex_is_inherit_option($font['line-height']))
				$out .= "\tline-height: " . invetex_prepare_css_value($font['line-height']) . ";\n";
			if (!empty($font['font-weight']) && !invetex_is_inherit_option($font['font-weight']))
			...
			$out .= ".{$slug}_margins() {\n";
			if (!empty($font['margin-top']) && !invetex_is_inherit_option($font['margin-top']))
				$out .= "\tmargin-top: " . invetex_prepare_css_value($font['margin-top']) . ";\n";
			if (!empty($font['margin-bottom']) && !invetex_is_inherit_option($font['margin-bottom']))
				$out .= "\tmargin-bottom: " . invetex_prepare_css_value($font['margin-bottom']) . ";\n";
			$out .= "}\n\n";
		}
		}
		...
		$custom_colors = invetex_get_custom_colors();
		if (is_array($custom_colors) && count($custom_colors) > 0) {
			foreach ($custom_colors as $scheme => $data) {
				if (is_array($data) && count($data) > 0) {
					foreach ($data as $key => $value) {
						if ($key == 'title' || invetex_strpos($key, 'bg_image')!==false) continue;
						$out .= "@{$scheme}_{$key}: " . esc_attr(
							!empty($value) 
								? $value
								: (invetex_strpos($key, 'bg_image')!==false
									? 'none'
									: 'inherit'
									)
							) . ";\n";
...
if (!function_exists('invetex_core_customizer_load_scripts')) {
	function invetex_core_customizer_load_scripts() {
		if (file_exists(invetex_get_file_dir('core/core.customizer/core.customizer.css')))
            wp_enqueue_style( 'invetex-core-customizer-style',	invetex_get_file_url('core/core.customizer/core.customizer.css'), array(), null);
		if (file_exists(invetex_get_file_dir('core/core.customizer/core.customizer.js')))
            wp_enqueue_script( 'invetex-core-customizer-script', invetex_get_file_url('core/core.customizer/core.customizer.js'), array(), null );
	}
}


// Prepare javascripts global variables for customizer admin page
if ( !function_exists( 'invetex_core_customizer_prepare_scripts' ) ) {
	function invetex_core_customizer_prepare_scripts() {
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_delete', 			esc_html__("Delete color scheme", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_delete_confirm',	esc_html__("Do you really want to delete this color scheme?", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_delete_complete',	esc_html__("Current color scheme is successfully deleted!", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_delete_failed',	esc_html__("Error while delete color scheme! Try again later.", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_copy', 			esc_html__("Copy color scheme", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_copy_confirm', 	esc_html__("Duplicate this color scheme?", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_copy_complete', 	esc_html__("Current color scheme is successfully duplicated!", 'invetex'));
		invetex_storage_set_array2('js_vars', 'to_strings', 'scheme_copy_failed', 		esc_html__("Error while duplicate color scheme! Try again later.", 'invetex'));
	}
}
...
if ( !function_exists( 'invetex_get_custom_fonts' ) ) {
	function invetex_get_custom_fonts() {
		return apply_filters('invetex_filter_get_custom_fonts', !invetex_storage_empty('custom_fonts') ? invetex_storage_get('custom_fonts') : array());
	}
}

// Add custom font parameters
if (!function_exists('invetex_add_custom_font')) {
	function invetex_add_custom_font($key, $data) {
		if (invetex_storage_empty('custom_fonts', $key)) invetex_storage_set_array('custom_fonts', $key, $data);
	}
}

// Return one or all font settings
if (!function_exists('invetex_get_custom_font_settings')) {
	function invetex_get_custom_font_settings($key, $param_name='') {
		return invetex_storage_get_array('custom_fonts', $key, $param_name);
	}
}

// Return fonts for css generator
if ( !function_exists( 'invetex_get_custom_fonts_properties' ) ) {
	function invetex_get_custom_fonts_properties() {
		$fnt = invetex_get_custom_fonts();
		$rez = array();
		foreach ($fnt as $k=>$f) {
			foreach ($f as $prop=>$val) {
				if ($prop == 'font-style') {
					if (invetex_strpos($val, 'i')!==false)
						$rez[$k.'_fl'] = 'italic';
					if (invetex_strpos($val, 'u')!==false)
						$rez[$k.'_td'] = 'underline';
				} else {
					$p = str_replace(
						array(
...
// Return fonts for css generator
if ( !function_exists( 'invetex_get_custom_font_css' ) ) {
	function invetex_get_custom_font_css($fnt) {
		$css = '';
		$fnt = invetex_storage_get_array('custom_fonts', $fnt);
		if (is_array($fnt)) {
			foreach ($fnt as $prop=>$val) {
				if (empty($val) || (invetex_strpos($prop, 'font-')===false && invetex_strpos($prop, 'line-')===false)) continue;
				if ($prop=='font-style') {
					if (invetex_strpos($val, 'i')!==false)
						$css .= ($css ? ';' : '') . $prop . ':italic';
					if (invetex_strpos($val, 'u')!==false)
...
// Return fonts for css generator
if ( !function_exists( 'invetex_get_custom_margins_css' ) ) {
	function invetex_get_custom_margins_css($fnt) {
		$css = '';
		$fnt = invetex_storage_get_array('custom_fonts', $fnt);
		if (is_array($fnt)) {
			foreach ($fnt as $prop=>$val) {
				if (empty($val) || invetex_strpos($prop, 'margin-')===false) continue;
				$css .= ($css ? ';' : '') . $prop . ':' . $val;
			}
		}
		return $css;
...
// Add color scheme
if (!function_exists('invetex_add_color_scheme')) {
	function invetex_add_color_scheme($key, $data) {
		if (invetex_storage_empty('custom_colors', $key)) invetex_storage_set_array('custom_colors', $key, $data);
	}
}

// Return color schemes
if ( !function_exists( 'invetex_get_custom_colors' ) ) {
	function invetex_get_custom_colors() {
		return apply_filters('invetex_filter_get_custom_colors', !invetex_storage_empty('custom_colors') ? invetex_storage_get('custom_colors') : array());
	}
}

// Return color schemes list, prepended inherit
if ( !function_exists( 'invetex_get_list_color_schemes' ) ) {
	function invetex_get_list_color_schemes($prepend_inherit=false) {
		$list = array();
		$colors = invetex_storage_get('custom_colors');
...
// Return scheme color
if (!function_exists('invetex_get_scheme_color')) {
	function invetex_get_scheme_color($clr_name='', $clr='') {
		if (empty($clr)) {
			$scheme = invetex_get_custom_option('body_scheme');
			if (empty($scheme) || invetex_storage_empty('custom_colors', $scheme)) $scheme = 'original';
			$clr = invetex_storage_get_array('custom_colors', $scheme, $clr_name);
		}
		return apply_filters('invetex_filter_get_scheme_color', $clr, $clr_name, $scheme);
	}
}

// Return scheme colors
if (!function_exists('invetex_get_scheme_colors')) {
	function invetex_get_scheme_colors($scheme='') {
		if (empty($scheme)) $scheme = invetex_get_custom_option('body_scheme');
		if (empty($scheme) || invetex_storage_empty('custom_colors', $scheme)) $scheme = 'original';
		return invetex_storage_get_array('custom_colors', $scheme);
	}
}
?>