<?php
/**
 * Invetex Framework: templates and thumbs management
 *
 * @package	invetex
 * @since	invetex 1.0
 */
...
// Theme init
if (!function_exists('invetex_templates_theme_setup')) {
	add_action( 'invetex_action_before_init_theme', 'invetex_templates_theme_setup' );
	function invetex_templates_theme_setup() {

		// Add custom thumb sizes into media manager
		add_filter( 'image_size_names_choose', 'invetex_show_thumb_sizes');
	}
}
...
if (!function_exists('invetex_add_template')) {
	function invetex_add_template($tpl) {
		...
		invetex_storage_set_array('registered_templates', $tpl['layout'], $tpl);
		if (!empty($tpl['thumb_title']) || !empty($tpl['thumb_slug']))
			invetex_add_thumb_sizes( $tpl );
		else 
			$tpl['thumb_title'] = '';
	}
}
...
if (!function_exists('invetex_get_template_name')) {
	function invetex_get_template_name($layout_name) {
		$tpl = invetex_storage_get_array('registered_templates', $layout_name);
		return $tpl['template'];
	}
}

// Return true, if template required content
if (!function_exists('invetex_get_template_property')) {
	function invetex_get_template_property($layout_name, $what) {
		$tpl = invetex_storage_get_array('registered_templates', $layout_name);
...
if (!function_exists('invetex_get_template_function_name')) {
	function invetex_get_template_function_name($layout_name) {
		$tpl = invetex_storage_get_array('registered_templates', $layout_name);
		return 'invetex_template_'.str_replace(array('-', '.'), '_', $tpl['template']).'_output';
	}
}

// Set template arguments
if (!function_exists('invetex_template_set_args')) {
	function invetex_template_set_args($tpl, $args) {
		invetex_storage_push_array('call_args', $tpl, $args);
	}
}


// Get template arguments
if (!function_exists('invetex_template_get_args')) {
	function invetex_template_get_args($tpl) {
		return invetex_storage_pop_array('call_args', $tpl, array());
	}
}


// Look for last template arguments (without removing it from storage)
if (!function_exists('invetex_template_last_args')) {
	function invetex_template_last_args($tpl) {
		$args = invetex_storage_get_array('call_args', $tpl, array());
...
if (!function_exists('invetex_add_thumb_sizes')) {
	function invetex_add_thumb_sizes($sizes) {
		static $mult = 0;
		if ($mult == 0) $mult = min(4, max(1, invetex_get_theme_option("retina_ready")));
		if (!isset($sizes['h_crop']))		$sizes['h_crop'] =  isset($sizes['h']) ? $sizes['h'] : null;
		if (empty($sizes['thumb_title']))	$sizes['thumb_title'] = invetex_strtoproper(!empty($sizes['layout']) ? $sizes['layout'] : $sizes['thumb_slug']);
		$thumb_slug = !empty($sizes['thumb_slug']) ? $sizes['thumb_slug'] : invetex_get_slug($sizes['thumb_title']);
		if (empty($sizes['layout']))		$sizes['layout'] = $thumb_slug;
		if (invetex_storage_get_array('thumb_sizes', $thumb_slug)=='') {
			invetex_storage_set_array('thumb_sizes', $thumb_slug, $sizes);
			// Register WP thumb size
			if (invetex_get_theme_setting('add_image_size') && (!isset($sizes['add_image_size']) || $sizes['add_image_size'])) {
				// Image thumb and retina version
				add_image_size( 'invetex-'.$thumb_slug, $sizes['w'], $sizes['h'], $sizes['h']!=null );
				if ($mult > 1)
					add_image_size( 'invetex-'.$thumb_slug.'-@retina', $sizes['w'] ? $sizes['w']*$mult : $sizes['w'], $sizes['h'] ? $sizes['h']*$mult : $sizes['h'], $sizes['h']!=null );
				// Cropped image thumb and retina version
				if ($sizes['h']!=$sizes['h_crop']) {
					add_image_size( 'invetex-'.$thumb_slug.'_crop', $sizes['w'], $sizes['h_crop'], true );
					if ($mult > 1) 
						add_image_size( 'invetex-'.$thumb_slug.'_crop-@retina', $sizes['w'] ? $sizes['w']*$mult : $sizes['w'], $sizes['h_crop'] ? $sizes['h_crop']*$mult : $sizes['h_crop'], true );
				}
			}
		}
	}
...
if (!function_exists('invetex_get_thumb_sizes')) {
	function invetex_get_thumb_sizes($opt) {
		$opt = array_merge(array(
			'layout' => 'excerpt',
			'thumb_slug' => ''
		), $opt);
		$tpl = invetex_storage_get_array('registered_templates', $opt['layout']);
		$thumb_slug = !empty($opt['thumb_slug']) 
						? $opt['thumb_slug'] 
						: (empty($tpl['thumb_slug']) 
							? (empty($tpl['thumb_title']) 
								? '' 
								: invetex_get_slug($tpl['thumb_title'])
								) 
							: invetex_get_slug($tpl['thumb_slug'])
							);
		$thumb_size = invetex_storage_get_array('thumb_sizes', $thumb_slug);
		return !empty($thumb_size) ? $thumb_size : array('w'=>null, 'h'=>null, 'h_crop'=>null);
	}
}

// Show custom thumb sizes into media manager sizes list
if (!function_exists('invetex_show_thumb_sizes')) {
	function invetex_show_thumb_sizes( $sizes ) {
		$thumb_sizes = invetex_storage_get('thumb_sizes');
