...
function invetex_theme_customizer(option, val) {
"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = INVETEX_STORAGE['co_add_params']!=undefined ? INVETEX_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#invetex-customizer-styles-'+option).length > 0 ? jQuery('#invetex-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="invetex-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
